...
def create_chat_tab(app_instance) -> List[gr.components.Component]:
	...
	def process_chat_message(
		...
		if "storage" in chat_state_value:
			chat_state_value["storage"].save_session(
				chat_state_value["session_id"],
	...
	async def execute_agent_task(
		...
		try:
			...
			async for output in app_instance.run_agent(
				message, max_steps, 4, provider, model, api_key, False, False
			):
	...
	def save_session(
		...
		try:
			if "storage" in chat_state_value:
				...
				return (
					gr.update(info=f"Session '{session_name}' saved successfully!"), 
					"",
	...
	clear_chat_btn.click(
		fn=clear_chat,
		inputs=[chat_state],
		outputs=[chat_history, chat_state, chat_input]
	)
	
	save_session_btn.click(
		fn=save_session,
		inputs=[session_name_input, chat_state],
		outputs=[session_name_input, session_name_input, session_list]
	)
	
	# Load session handler
	load_session_btn.click(
		fn=load_session,
		inputs=[session_list, chat_state],
		outputs=[chat_history, chat_state, chat_input]
	)
