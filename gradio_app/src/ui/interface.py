...
def create_configuration_tab(app_instance) -> List[gr.components.Component]:
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            try:
                from ..models.llm_models import check_provider_availability, PROVIDER_CONFIGS
                provider_choices = []
            ...
            provider_info = gr.HTML(
                value=_get_provider_info_html(default_provider),
            ...
            model_recommendations = gr.HTML(
                value=_get_model_recommendations_html(default_provider, default_model),
        ...
        with gr.Column(scale=1):
            ...
            # API key help links
            api_help = gr.HTML(
                value=_get_api_help_html(default_provider),
                label="Get API Key"
            )
            
            # Provider status
            provider_status = gr.HTML(
                value="<div style='color: green;'>✅ Provider ready</div>",
                label="Status"
            )
    ...
    return [
        llm_provider, llm_model, api_key, share_terminal,
        ...
        enable_cost_optimization, prefer_free_models, max_cost_per_request, refresh_models_btn
    ]
