...
MODEL_CATEGORIES = {
	...
	'DeepSeek': {'latest': ['deepseek-chat', 'deepseek-reasoner'], 'specific': ['deepseek-v3', 'deepseek-r1']},
}
...
def get_llm(provider: str, model: str, api_key: str = None) -> Optional[object]:
	...
	try:
		if provider == 'OpenAI':
			...
			return ChatOpenAI(model=model, api_key=SecretStr(api_key), base_url='https://api.deepseek.com/v1')
