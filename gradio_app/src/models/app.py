...
class MacOSUseGradioApp:
	def __init__(self):
		...
		storage_dir = Path(__file__).parent.parent.parent / '.context_storage'
		self.context_manager = ContextBucketManager(storage_dir)
	...
	async def add_context_item(
		self, session_id: str, item_type: str, title: str, content: str, priority: str = 'medium', tags: List[str] = None
	) -> bool:
		"""Add a context item to the specified session's context bucket"""
		try:
			...
			item = await context_bucket.add_item(
				type=ContextItemType(item_type),
				title=title,
				content=content,
			...
			logging.error(f'Error adding context item: {e}')
			return False

	async def get_context_items(self, session_id: str) -> List[dict]:
		"""Get all context items for the specified session"""
		try:
			context_bucket = await self.context_manager.get_context_bucket(session_id=session_id)
			...
			logging.error(f'Error getting context items: {e}')
			return []

	async def remove_context_item(self, session_id: str, item_id: str) -> bool:
		"""Remove a context item from the specified session's context bucket"""
		try:
			context_bucket = await self.context_manager.get_context_bucket(session_id=session_id)
			context_bucket.remove_item(item_id)
			await self.context_manager.save_context_bucket()
			return True
		except Exception as e:
			logging.error(f'Error removing context item: {e}')
			return False

	async def clear_context_items(self, session_id: str) -> bool:
		"""Clear all context items for the specified session"""
		try:
			context_bucket = await self.context_manager.get_context_bucket(session_id=session_id)
			...
			logging.error(f'Error clearing context items: {e}')
			return False
	...
	async def run_automation(
		...
		try:
			for i, agent_config in enumerate(automation['agents']):
				...
				# Initialize agent with context bucket
				session_id = f'automation_{automation_name}_{i}'
				self.agent = await create_agent_with_context(
				...
				try:
					...
					yield (''.join(self.terminal_buffer), gr.update(interactive=True), gr.update(interactive=False))
					break
			...
			yield (error_msg, gr.update(interactive=True), gr.update(interactive=False))
			self._cleanup_state()
		...
		try:
			...
			if share_prompt and not share_terminal:
				try:
					...
					logging.info(
						f'Immediately sending prompt only to Google Form (share_prompt={share_prompt}, share_terminal={share_terminal})'
					)
			...
			# Initialize agent with context bucket
			session_id = f'gradio_{hash(task)}_{llm_provider}_{llm_model}'
			self.agent = await create_agent_with_context(
				task=task,
				llm=llm,
				session_id=session_id,
